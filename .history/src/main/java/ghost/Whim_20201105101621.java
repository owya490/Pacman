...
public class Whim extends GhostObject {
    ...
    public Whim(int x, int y, PImage ghost, PImage ghost_scared, ArrayList<Integer> modeLengths1, int speed, boolean[][] path, Player player_g, ArrayList<GhostObject> other_ghosts){
    ...
    public char[] upOrDown() {
        ...
        int x_diff = player_x - this.current_chaser.getX();
        int y_diff = player_y - this.current_chaser.getY();

        this.x_dest = this.current_chaser.getX() + x_diff;
        this.y_dest = this.current_chaser.getY() + y_diff;


        int x_diff = this.x - this.x_dest;
        int x_diff_f = Math.abs(x_diff);
        int y_diff = this.y - this.y_dest;
        int y_diff_f = Math.abs(y_diff);
