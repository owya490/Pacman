...
public class Ghost {
    ...
    public Ghost(int x, int y, PImage ghost, JSONArray modeLengths, int speed, boolean[][] path, Player player_g){
        ...
        this.arr = new String[modeLengths.length()];

        for (int i = 0; i < modeLengths.length; i ++) {
            arr[i] = modeLengths.optString(i);
        }

    }

    public boolean setScatterFalse() {
        this.scatter = false;
        return false;
    }
    ...
    public void tick() {
        this.frames ++;
        // if (this.frames / 60 == (int) modeLengths[0]) {
        //     System.out.println(setScatterFalse());
        // }
