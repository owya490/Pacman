...
public class MazeBuilder extends PApplet{
    ...
    public ArrayList<Fruit> superFruitList;


    public MazeBuilder(String filename, PImage downLeft, PImage downRight, PImage horizontal, PImage upLeft, PImage upRight, PImage vertical, PImage fruit, PImage superFruit) {
        ...
        for (int y = 0; y < this.lines.size(); y ++) {
            for (int x = 0; x < this.lines.get(y).length; x ++) {
                if (this.lines.get(y)[x].equals("8")) {
                    superFruit tmp = new superFruit(x*16, y*16, this.fruit);
                    //System.out.println("f");
                    tmpList.add(tmp);
                }
            }
