...
public class App extends PApplet {
    ...
    public void setup() {
        ...
        this.player = new Player(this.lives, this.speed, this.map, this.playerClosed, this.playerDown, this.playerLeft, this.playerRight, this.playerUp);
        ...
        for (int x = 0; x < this.ghost_x.size(); x ++) {
            for (int y = 0; y < this.ghost_y.size(); y ++){
                this.ghost_obj.add(new Ghost(this.ghost_x.get(x), this.ghost_x.get(y), this.ghost, this.modeLengths));
                continue;
            }
        }
