...
public class MazePath {
    ...
    public static void createPath(String[][] maze) {
        this.maze_final = new boolean[maze.length][maze[0].length];
            ...
            for (int x = 0; x < maze[1].length; x ++) {
                ...
                if (maze[y][x].equals("7")) {
                    for (int y1 = ((y*16) - 5); y1 < ((y*16) + 11); y1 ++) {
                        this.maze_final[y1][x*16] = true;
                    }
                    for (int x1 = (x*16); x1 < ((x*16) + 16); x1 ++) {
                        this.maze_final[(y*16)-5][x1] = true;
                    }
                }
            }
        }
